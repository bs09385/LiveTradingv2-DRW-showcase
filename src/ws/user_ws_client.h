...
struct UserWsClientConfig {
    std::string endpoint;
    std::string api_key;
    std::string api_secret;
    std::string api_passphrase;
    std::vector<std::string> market_ids;  // condition IDs
    int64_t ping_interval_ms = 10000;
    int64_t reconnect_base_ms = 1000;
    int64_t reconnect_max_ms = 30000;
    int64_t stale_threshold_ms = 30000;  // stale watchdog subsumes pong timeout
...
class UserWsClient {
public:
    explicit UserWsClient(const UserWsClientConfig& config);
    ~UserWsClient();
    ...
    // Dynamic subscription: add condition_ids to existing connection.
    // Thread-safe: posts to io_context for serialized async_write.
    void send_subscribe_add(const std::vector<std::string>& condition_ids);

    // Dynamic unsubscribe: remove condition_ids from existing connection.
    // Thread-safe: posts to io_context for serialized async_write.
    void send_unsubscribe(const std::vector<std::string>& condition_ids);
